...
PetscErrorCode TopOpt::SetUp(){
	...
	eta      = 0.5;		// Value of the threshold
	IterProj = 40 ; 	// Iterations by step
	...
	Rmax     = (3.0/96.0*3.01)   	; // Maximum size radius 
	Rint     = (3.0/96.0*1.01) 	 	; // Minimum size radius (inner ring)
	RmaxDil  = Rmax + Rint/2.0 	 	; // Maximum size radius in dilated design
	RintDil  = Rint + Rint/2.0 	 	; // Minimum size radius in dilated design
	epsiMS   = 0.05  			 				; // Epsilon parameter
	pagg	   = 100.0 	 		 				; // Aggregation exponent
	Nagg     = 96.0*16.0*32.0  	 	; // number of data (for p-mean)
	...
	volfrac 		= 0.5   				; 				// Volume fraction on Dilated (it's scaled on the main file)
	volfracREF	= volfrac				; 				// Volume fraction in Intermediate
	maxItr  		= 359   				; 				// Maximum number of iterations (check IterProj above: 40*9=360)
	rmin    		= (3.0/96.0*1.01)*2.0 ; 	// Filter radius (it is not the minimum size radius)
	Emin    		= 1.0e-6 	 			; 				// 
	Emax    		= 1.0   				;
